                        ...
                        if ~isempty(list) % if that session exists. 
                            for i_list = 1:1:length(list) % iterate through all structures 
                                if i_prepost == 1
                                    ...
                                    %median_base_temp = median(signal_save(4000:5001));
                                    temp_signal = [temp_signal; signal_save'];
                                    %median_base = [median_base; median_base_temp]
                                end
                                if i_prepost == 2
...
%% Now ADD tbl_wake-nrem
nonan_Tbl.sub = tbl.sub; 

%% LMM-- peak and mean for session averages 
...
% Full model
full = fitlme(nonan_Tbl,'peak ~ prepost + earlylate + sub + (1|mouse)'); % removed 1|session to be consistent with event based
disp(full)

% ~ PREPOST ~
prepost = fitlme(nonan_Tbl,'peak ~ prepost + (1|mouse)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(prepost)
noprepost = fitlme(nonan_Tbl,'peak ~ earlylate + sub + (1|mouse)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noprepost);
% base vs. prepost
...
disp(earlylate)
noearlylate = fitlme(nonan_Tbl,'peak ~ prepost + sub + (1|mouse)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noearlylate);
% base vs. prepost
...
% ~ sub ~
sub = fitlme(nonan_Tbl,'peak ~ sub + (1|mouse)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(sub)
nosub = fitlme(nonan_Tbl,'peak ~ prepost + earlylate + (1|mouse)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nosub);
% base vs. prepost
compare(base, sub,'nsim',1000)
% full vs. noprepost 
compare(nosub,full,'nsim',1000)
