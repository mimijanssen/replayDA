...
cd D:\SWR_DA_MegaMatrix_1s_basic
...
%% Basic models
lme_base = fitlme(ProcPeakTbl,'Peak ~ 1 + (1|mouseID) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_base)

lme_ba = fitlme(ProcPeakTbl,'Peak ~ BeforeAfter + (1|mouseID) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_ba)

compare(lme_base, lme_ba)
% Peak after is higher and adding it to the models helps 

lme_pp = fitlme(ProcPeakTbl,'Peak ~ PrePost + (1|mouseID) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_pp)
compare(lme_base, lme_pp)

% Prepost helps the model and the peak is lower for post. 

% Full model 
lme_full = fitlme(ProcPeakTbl,'Peak ~ BeforeAfter + PrePost + (1|mouseID) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_full)
% both are signitificant 
compare(lme_pp, lme_full)
compare(lme_ba, lme_full)
% full model is better

% MODEL for power and other descriptors 
lme_full2 = fitlme(ProcPeakTbl,'Peak ~ SWRdur + SWRpower + BeforeAfter + PrePost + (1|mouseID) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_full2)
