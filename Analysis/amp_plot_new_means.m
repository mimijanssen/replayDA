...
%% Observed Stats
obs_sal = zeros(length(fields_saline),1);
obs_amp = zeros(length(fields_amp),1);
for i = 1:length(fields_saline)
    name = fields_saline{i};
    sal = Saline_sess.(name);
    amp = Amp_sess.(name);
    obs_sal(i) = mean( ...
        mean(sal(:,samples_before+1:end),2) ...
        - sal(:,samples_before));
    obs_amp(i) = mean( ...
        mean(amp(:,samples_before+1:end),2) ...
        - amp(:,samples_before));
end
[~,~,~,stats] = ttest2(obs_sal,obs_amp);
obs_t = stats.tstat;
...
traceLength = size(Saline_sess.(fields_saline{1}),2);
validStart = samples_before + 1;
validEnd   = traceLength - samples_after;
for p = 1:nPerm
    perm_sal = zeros(length(fields_saline),1);
    perm_amp = zeros(length(fields_amp),1);

    %% ---------- Saline ----------
    for m = 1:length(fields_saline)
        name = fields_saline{m};
        sess = Saline_sess.(name);
        sessionVals = zeros(size(sess,1),1);
        for s = 1:size(sess,1)
            x = sess(s,:);
            %% random circular shift
            shift = randi(traceLength);
            x = circshift(x,shift);
            %% random mock injection
            inj = randi([validStart validEnd]);
            postMean = mean(x(inj+1:inj+samples_after));
            inst = x(inj);
            sessionVals(s) = postMean - inst;
        end
        perm_sal(m) = mean(sessionVals);
    end

    %% ---------- Amphetamine ----------
    for m = 1:length(fields_amp)
        name = fields_amp{m};
        sess = Amp_sess.(name);
        sessionVals = zeros(size(sess,1),1);
        for s = 1:size(sess,1)
        ...
        perm_amp(m) = mean(sessionVals);
    end
    [~,~,~,stats] = ttest2(perm_sal,perm_amp);
    perm_t(p) = stats.tstat;
end
%% 
p_perm = (sum(abs(perm_t) >= abs(obs_t)) + 1) / (nPerm + 1);

fprintf('Observed t = %.3f\n',obs_t)
fprintf('Permutation p = %.4f\n',p_perm)

%%
figure
histogram(perm_t,30)
hold on
xline(obs_t,'r','LineWidth',3)
xlabel('t statistic')
ylabel('Count')
title(sprintf('Permutation test (p = %.4f)',p_perm))
