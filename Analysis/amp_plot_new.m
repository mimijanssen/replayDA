...
max_saline = max(avg_sal(:,1:samples_before),[],2); 
max_amp =  max(avg_amp(:,1:samples_before),[],2); 

% 2) find the min value in the post injection period: all samples after
% samples before
min_saline = max(avg_sal(:,samples_before+1:end),[],2); 
min_amp =  max(avg_amp(:,samples_before+1:end),[],2); 
...
for iter = 1:length(fields_saline) % iterate through each mouse -- same for amphetamine...
    ...
    sal_sess_dF = max(data_sal_sess(:,samples_before+1:end),[],2) - max(data_sal_sess(:,1:samples_before),[],2); 
    cat_sal_sess = count*ones(1,numel(sal_sess_dF));

    count = count + 1;
    data_amp_sess = Amp_sess.(fields_saline{iter}); 
    amp_sess_dF = max(data_amp_sess(:,samples_before+1:end),[],2) - max(data_amp_sess(:,1:samples_before),[],2); 
...
for iter = 1:length(fields_saline) % iterate through each mouse -- same for amphetamine...
    ...
    % find the min and max values 
    sal_sess_dF = max(data_sal_sess(:,samples_before+1:end),[],2) - max(data_sal_sess(:,1:samples_before),[],2); % 2x1
