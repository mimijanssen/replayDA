...
mean_med = zeros(n2,1);
for i_med = 1:1:n2
    mean_med(i_med,:) = mean(med(i_med,x_values));
end
...
[mean_h, mean_p,~, mean_tstats] = ttest(mean_high, mean_low); 
[dF_h, dF_p,~, dF_tstats] = ttest(dF_high, dF_low); 

%%
cd 'F:\M548\avg_data\RPE_ttest';
filename = append(file_name, "RPE_Ttest.mat");
save(filename, 'mean_h','mean_p','mean_tstats','auc_h','AUC_tstats','auc_p','dF_h','dF_p', 'dF_tstats','dF_high','dF_med','dF_low','AUC_high', 'AUC_low','mean_low','mean_med','mean_high');
