...
%P = readtable('M646_2026_02_01_VT1-convertedDLC_resnet50_Linear TrackApr5shuffle1_100000.csv','PreserveVariableNames',true); % CHANGE THIS 
...
figure (1)
plot(zscore(linspd.data)) % 0.5? z-score
...
sample_video = median(diff(Timestamps))/1e6 ; % 1 s / average time between samples 
...
%% sandbox 
%figure(2)
%plot(A(:,20),A(:,21),'.'); hold on; plot(A(:,23),A(:,24),'.'); plot(A(:,26),A(:,27),'.'); plot(A(:,29),A(:,30),'.'); plot(A(:,32),A(:,33),'.'); plot(A(:,35),A(:,36),'.'); plot(A(:,38),A(:,39),'.'); plot(A(:,41),A(:,42),'.'); 
%legend

%% start matrix
% each swr has it's own row 
matrix_sess = array2table(zeros(length(SWR_ind_mid),28),'VariableNames',{'mouseID','sess','swrID','PrePost','TwosPreRaw','TwosTrackRaw','TwosPostRaw','TwosPreProc','TwosTrackProc','TwosPostProc','OnesBeforePeak','OnesAfterPeak','OnesBeforeAUC','OnesAfterAUC','TimeAfterPeak','OnesBeforePeakRAW','OnesAfterPeakRAW','OnesBeforeAUCRAW','OnesAfterAUCRAW','SWRdur','SWRamp','SWRpower','SWRtimestart','SWRtimeend','SWRindmid','SWRindstart','SWRindend','immobile'});
...
linspd.data = zscore(linspd.data);

seconds = 4;
samples = seconds * FP.cfg.hdr{1,1}.SamplingFrequency;
samples_speed = seconds * round(fs_video);  % how do i find this
...
for i = 1:height(matrix_sess)
    ...
    speed_index = nearest_idx3(swr_time, linspd.tvec);
    % Require a complete window on both sides of the SWR
    if (fiber_index - samples < 1) || ...
       (fiber_index + samples > length(FP.data))
        badSWR(i) = true;
    end
    if (speed_index - samples_speed < 1) || ...
       (speed_index + samples_speed > length(linspd.data))
       badSWR(i) = true;
     end
end
...
for i = 1:height(matrix_sess)
    ...
    idx_speed = (speed_index-samples_speed):(speed_index+samples_speed);
    ...
    speed_struct = linspd.data(idx_speed);
    ...
    matrix_sess.speed8s{i} = speed_struct;
    if all(speed_struct < 0.5,'all')
        disp('immobile swr')
        matrix_sess.immobile(i) = 1;
    else
        matrix_sess.immobile(i) = 2;
    end
end
% YAYYYYY

%% Plot the speed and what I determined was movement anything above 30% of the median signal? 
