...
for s = 1:length(structure_names)
    ...
    for i = 1:num_sessions
        ...
        % Pre-condition SWR-DA strength
        RPE_strength_dF_all(1,i) = max(curr_structure.(session).AUC_tstats.tstat);%dF_tstats.tstat); %p;
...
exportgraphics(gcf,'ValuevsSWRDA_Pre_AUC.png','ContentType','vector'); 
...
cd 'C:\Users\mimia\Desktop\PrePrint Figures'
exportgraphics(gcf,'ValuevsSWRDA_Post_AUC.png','ContentType','vector'); 
