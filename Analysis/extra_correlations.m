...
structure_names = {'M433sd', 'M460sd', 'M453sd', 'M533sd', 'M534sd','M545sd','M547sd','M548sd'}; % Add all your structure names here
...
for s = 1:length(structure_names)
    ...
    for i = 1:num_sessions
        ...
        SWR_DA_strength.pre(1,i) = max(curr_structure.(session).avg_fiber_pre - curr_structure.(session).circ_std_pre);
        
        % Post-condition SWR-DA strength
        SWR_DA_strength.post(1,i) = max(curr_structure.(session).avg_fiber_post - curr_structure.(session).circ_std_post);
    end
...
% List of structure names
structure_names = {'M433rpe', 'M460rpe', 'M453rpe', 'M533rpe', 'M534rpe','M545rpe','M547rpe','M548rpe'}; % Add all your structure names here
...
for s = 1:length(structure_names)
    ...
    RPE_strength = zeros(1, num_sessions); % Initialize RPE values
    ...
    for i = 1:num_sessions
        ...
        % Pre-condition SWR-DA strength
        RPE_strength(1,i) = max(curr_structure.(session).avg_high) - min(curr_structure.(session).avg_low);
 
    end
    
    % Save the results in SWR_DA_strength_all under the current structure name
    RPE_strength_all.(curr_structure_name) = RPE_strength;
end
...
structure_names = {'M433rpe', 'M460rpe', 'M453rpe', 'M533rpe', 'M534rpe','M545rpe','M547rpe','M548rpe'}; % Add all your structure names here
structure_names_swrda = {'M433sd', 'M460sd', 'M453sd', 'M533sd', 'M534sd','M545sd','M547sd','M548sd'}; % Add all your structure names here

% Colors for each structure (use distinguishable colors)
colors = lines(length(structure_names)); % Generates a distinct color for each structure
...
ylabel('RPE Strength');
title('Pre-Track Rest SWR-DA Strength vs RPE Strength');
hold off;
