...
matrix_sess = array2table(zeros(length(SWR_ind_mid),13),'VariableNames',{'mouseID','sess','swrID','PrePost','8sPreRaw','8sPostRaw','8sPreProc','8sPostProc','4sPrePeak','4sPostPeak','4sPreAUC','4sPostAUC','TimePostPeak'});
...
matrix_sess.('PrePost')(1:SWR_ind_mid_post-1,1) = 1;
% 2 for post. 
matrix_sess.('PrePost')(SWR_ind_mid_post:end,1) = 2;

%% populate matrix with structure information. 
% WORKING ON THIS NOW
matrix_sess.('8sPreRaw') = cell(height(matrix_sess), 1);


for i = 1:height(matrix_sess)
    % Create a structure with your data for each row
    raw_data_struct = struct();
    raw_data_struct.signal = rand(1, 8000); % Replace with your actual 1x8000 data
    raw_data_struct.metadata = "Sample metadata"; % Add additional fields if needed
    
    % Assign the structure to the corresponding cell in the table
    matrix_sess.('8sPreRaw'){i} = raw_data_struct;
end

%% access that information 
row_index = 1; % Replace with the desired row index
raw_data_struct = matrix_sess.('8sPreRaw'){row_index};

% Access fields within the structure
signal = raw_data_struct.signal;
metadata = raw_data_struct.metadata;



%%
matrix_sess.mouseID = your_mouseID_data; % Replace with a vector of mouse IDs
