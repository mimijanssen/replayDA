...
% REPLOTTING TO account for circshift means
...
updated_pre_heatmap_sorted = sortrows(updated_map_pre, 5001, 'descend');
updated_post_heatmap_sorted = sortrows(updated_map_post, 5001, 'descend');
...
for iter1 = 1:1: length(circ_std_fiber_pre)
    for iter_rows = 1:1:length(pre_SWR_ind)
        if zdF_extract_pre_con(iter_rows,iter1) > 0
            updated_map_pre_con(iter_rows,iter1) = zdF_extract_pre_con(iter_rows,iter1) - circ_std_fiber_pre(iter1); % this is subtracting by 2 stds...
        else
...
hold on
plot(time_extract_pre(1,:),(avg_fiber_pre- circ_avg_fiber_pre),'LineWidth',2,'Color',low_c) % subtrack the circ_avg_pre here
xl = xline(4,'-',{'SWR'});
...
hold on
plot(time_extract_post(1,:),(avg_fiber_post-circ_avg_fiber_post),'LineWidth',2,'Color',low_c)
hold on
xl = xline(4,'-',{'SWR'});
xl.LabelVerticalAlignment = 'top';
%hold off
...
fig2.WindowState = 'maximized';
 %%
  ...
  cd 'C:\Users\mimia\OneDrive\MVDM\session plots\hyp plots\M433'
  % save descriptive plot
  saveas(fig2,'M433_recording1_hypothesis.png') % CHANGE THIS 
