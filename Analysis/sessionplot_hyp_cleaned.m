...
left  = evt3.t{1}- csc.tvec(1);
right = evt3.t{2}- csc.tvec(1);
evt_all = sort([left right]);
evt_ordered = evt_all(1:min(60,length(evt_all)));
length(evt_ordered)
...
valid_evt = (SWR_fiber_ind_post - samples > 0) & ...
            (SWR_fiber_ind_post + samples <= length(time));

SWR_fiber_ind_post = SWR_fiber_ind_post(valid_evt);
post_SWR_ind = post_SWR_ind(valid_evt);
% ---------------------------------------------------------
...
for ievt = 1:1:length(post_SWR_ind)-1 % -2 for M654 track 1 ; -3 for M600 recording 1; -3 for M646 recording 7; and M654 recording 6 -4 for M646 recording 4; -1 for everyone else. !!! start from 3 for recording 2 M654
...
for iter_circ = 1:1:N 
    ...
    for ievt = 1:1:length(post_SWR_ind)-1 % ALSO CHANGE HERE!!!!!
...
% ---------------------------------------------------------
% REMOVE EVENTS TOO CLOSE TO BEGINNING OR END
valid_evt = (SWR_fiber_ind_pre - samples > 0) & ...
            (SWR_fiber_ind_pre + samples <= length(time));

SWR_fiber_ind_pre = SWR_fiber_ind_pre(valid_evt);
pre_SWR_ind = pre_SWR_ind(valid_evt);
% ---------------------------------------------------------
...
for ievt = 1:1:length(pre_SWR_ind)-1  % 1 for all sessions except for M646 recording 5 (start at 2.) ; and M654 recording 2 start from 3
...
% REMOVE EVENTS WITHIN 8s AFTER PHOTOBEAM BREAK
window = 8; % seconds

remove_evt = false(length(SWR_time_mid_track),1);

for i = 1:length(SWR_time_mid_track)
    
    % time difference between this SWR and all beam breaks
    dt = SWR_time_mid_track(i) - photobeam_times;
    
    % check if SWR occurs within 0-8 seconds after any break
    if any(dt >= 0 & dt <= window)
        remove_evt(i) = true;
    end
    
end

% keep only events NOT within 8s of beam break
SWR_time_mid_track(remove_evt) = [];
SWR_fiber_ind_track(remove_evt) = [];
track_SWR_ind(remove_evt) = [];

% ---------------------------------------------------------
% REMOVE EVENTS TOO CLOSE TO BEGINNING OR END
valid_evt = (SWR_fiber_ind_track - samples > 0) & ...
            (SWR_fiber_ind_track + samples <= length(time));

SWR_fiber_ind_track = SWR_fiber_ind_track(valid_evt);
track_SWR_ind = track_SWR_ind(valid_evt);
% ---------------------------------------------------------
...
avg_fiber_track = (zdF_extract_track); % not mean for M646 day 3. 
...
hold on
plot(time_extract_pre(2,:),avg_fiber_pre,'LineWidth',2,'Color',low_c)
xl = xline(5,'-',{'SWR'});
...
subplot(3,4,9:10)
shadedErrorBar(time_extract_pre(1,:),circ_avg_fiber_post,circ_std_fiber_post,'lineProps','-k','transparent',1) % subtracted the circ mean here
hold on
plot(time_extract_pre(1,:),circ_avg_fiber_post,'LineWidth',2,'Color','k') %subtracted the circ mean here
hold on
plot(time_extract_pre(1,:),avg_fiber_post,'LineWidth',2,'Color',low_c)
hold on
xl = xline(5,'-',{'SWR'});
xl.LabelVerticalAlignment = 'top';
%hold off
...
subplot(3,4,11)
imagesc(post_heatmap) %updated_post_heatmap_sorted) 
colorbar
xticks([0 8000 16000])
xticklabels({'-5','0','5'})
xlabel('Time from SWR (s)')
