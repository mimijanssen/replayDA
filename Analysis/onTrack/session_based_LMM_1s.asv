...
%         end
%     elseif ProcPeakTbl.PrePost(i) == 2 % track
%         if ProcPeakTbl.BeforeAfter(i) == 'OnesBeforePeak' % before swr
%             x2 = (length(ProcPeakTbl.TwosTrackRaw{i,1}.signal) - 1)/2;
%             x1 = 1;
%             subset_signal = ProcPeakTbl.TwosTrackProc{i,1}.signal(x1:x2); 
%             subset_time = ProcPeakTbl.TwosTrackProc{i,1}.tvec(x1:x2) - ProcPeakTbl.TwosTrackProc{i,1}.tvec(x2+1);
%             [max_before, I_before] = max(subset_signal);
%             Peak_one_sec = [Peak_one_sec; max_before];
%             Time_one_sec = [Time_one_sec; subset_time(I_before)];
%         elseif ProcPeakTbl.BeforeAfter(i) == 'OnesAfterPeak' % after swr
%             x3 = ((length(ProcPeakTbl.TwosTrackRaw{i,1}.signal) - 1)/2);
%             x4 = length(ProcPeakTbl.TwosTrackRaw{i,1}.signal);
%             subset_signal = ProcPeakTbl.TwosTrackProc{i,1}.signal(x3:x4);
%             subset_time =  ProcPeakTbl.TwosTrackProc{i,1}.tvec(x3:x4) - ProcPeakTbl.TwosTrackProc{i,1}.tvec(x2+1); 
%             [max_after, I_after] = max(subset_signal); % finds the max signal one second before an swr 
%             Peak_one_sec = [Peak_one_sec; max_after];
%             Time_one_sec = [Time_one_sec; subset_time(I_after)];
%         end
%     elseif ProcPeakTbl.PrePost(i) == 3 % post track
%         if ProcPeakTbl.BeforeAfter(i) == 'OnesBeforePeak' % before swr
...
for i = 1:height(ProcPeakTbl)
    ...
    if ProcPeakTbl.PrePost(i) == 1
        raw = ProcPeakTbl.TwosPreRaw{i,1};
        proc = ProcPeakTbl.TwosPreProc{i,1};

    elseif ProcPeakTbl.PrePost(i) == 2
        raw = ProcPeakTbl.TwosTrackRaw{i,1};
        proc = ProcPeakTbl.TwosTrackProc{i,1};

    elseif ProcPeakTbl.PrePost(i) == 3
        raw = ProcPeakTbl.TwosPostRaw{i,1};
        proc = ProcPeakTbl.TwosPostProc{i,1};
    end

    % -------------------------
    % SKIP EMPTY SIGNALS
    % -------------------------
    if isempty(raw) || isempty(raw.signal)
        continue
    end
    ...
    % center split (your original logic)
    x2 = floor((length(sig)-1)/2);
    x1 = 1;
    x3 = x2;
    x4 = length(sig);
    ...
    % BEFORE PEAK
    % -------------------------
    if ProcPeakTbl.BeforeAfter(i) == 'OnesBeforePeak'

        subset_signal = proc.signal(x1:x2);

        [max_val, idx] = max(subset_signal);

        subset_time = tvec(x1:x2) - tvec(x2+1);
    ...
    % AFTER PEAK
    % -------------------------
    elseif ProcPeakTbl.BeforeAfter(i) == 'OnesAfterPeak'

        subset_signal = proc.signal(x3:x4);

        [max_val, idx] = max(subset_signal);

        subset_time = tvec(x3:x4) - tvec(x3+1);
...
% REMOVE INVALID ROWS
% -------------------------
ProcPeakTbl = ProcPeakTbl(keep_row,:);
...
for i = 1:height(ProcPeakTbl)
    ...
    if ProcPeakTbl.PrePost(i) == 1
        valid_rows(i) = ~(isempty(ProcPeakTbl.TwosPreRaw{i,1}) || ...
                          isempty(ProcPeakTbl.TwosPreRaw{i,1}.signal));

    elseif ProcPeakTbl.PrePost(i) == 2
        valid_rows(i) = ~(isempty(ProcPeakTbl.TwosTrackRaw{i,1}) || ...
                          isempty(ProcPeakTbl.TwosTrackRaw{i,1}.signal));

    elseif ProcPeakTbl.PrePost(i) == 3
        valid_rows(i) = ~(isempty(ProcPeakTbl.TwosPostRaw{i,1}) || ...
                          isempty(ProcPeakTbl.TwosPostRaw{i,1}.signal));
    end
end


%%
ProcPeakTbl = ProcPeakTbl(valid_rows,:);
...
Tbl.PrePost = categorical(Tbl.PrePost);
Tbl.mouseID = categorical(Tbl.mouseID);
Tbl.BeforeAfter = categorical(Tbl.BeforeAfter);

Tbl.PrePost = reordercats(Tbl.PrePost, ...
    ['2'; setdiff(categories(Tbl.PrePost), {'2'})]);
%
lme_swrevt4 = fitlme(Tbl,'Peak ~ PrePost + (1|mouseID) + (1|sess:mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_swrevt4)

lme_swrevt5 = fitlme(Tbl,'Peak ~ BeforeAfter + PrePost + (1|mouseID) + (1|sess:mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(lme_swrevt5)

compare(lme_swrevt5, lme_swrevt4, 'nsim',1000)
...
uni_prepost = unique(Tbl.PrePost);  % Should be [1,2,3] for Pre/Post
...
for i_mouse = 1:length(uni_mouse)
    for i_sess = 1:length(uni_sess)
            ...
            for i_beforeafter = 1:length(uni_swr) % loops through before first and then after...  
                ...
                if ~isempty(list) % if that session exists. 
                    for i_list = 1:1:length(list) % iterate through all structures 
                        ...
                        if i_prepost == 2
                            signal_save = Tbl.TwosTrackProc{list(i_list),1}.signal;
                            temp_signal = [temp_signal; signal_save'];
                        end
                        if i_prepost == 3
        ...
        if ~isempty(list_allsess) % if that session exists 
            avg_peak = mean(Tbl.Peak_one_sec(list,1));
            avg_time = mean(Tbl.Time_one_sec(list,1));
...
%% oop
%sess_avg_tbl2.peak = sess_avg_tbl2.peak{:,:};
%sess_avg_tbl2.peak = sess_avg_tbl2.time{:,:};
...
%%
sess_avg_tbl2.prepost = reordercats(sess_avg_tbl2.prepost, ...
    ['2'; setdiff(categories(sess_avg_tbl2.prepost), {'2'})]);

lme_beforeafter2 = fitlme(sess_avg_tbl2,'peak ~ beforeafter*prepost + (1|mouse) + (1|sess)'); 
...
for m = 1:length(uni_mouse) % iterate over mice
        ...
        temptable = sess_avg_tbl2(mouseI,:);
        signal_matrix = (temptable.signal);  % results in an N x 3201 matrix
        mean_signal = mean(signal_matrix, 1);  % 1 x 3201
...
%% PLOT THAT WOOHOOO
common_tvec = linspace(-1, 1, 3201); % Modify based on your data
grand_mean_signal = mean(grand_signal);
grand_SEM_signal = std(grand_signal)/sqrt(4); 
figure(1)
