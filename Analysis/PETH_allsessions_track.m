                    ...
                    try
                        ...
                        if ~isempty(list) % if that session exists. 
                            for i_list = 1:1:length(list) % iterate through all structures 
                                ...
                                if i_prepost == 2
                                    signal_save = allTables2.TwosTrackProc{list(i_list),1}.signal; % signals are saved in different columns but I'm saving everything 
                                    temp_signal = [temp_signal; signal_save'];
                                end
                                if i_prepost == 3
...
% Full model
full = fitlme(nonan_Tbl,'peak ~ prepost + earlylate + (1|mouse) + (1|sess)'); 
disp(full)

% ~ PREPOST ~
prepost = fitlme(nonan_Tbl,'peak ~ prepost + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(prepost)
noprepost = fitlme(nonan_Tbl,'peak ~  earlylate + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noprepost);
% base vs. prepost
...
disp(earlylate)
noearlylate = fitlme(nonan_Tbl,'peak ~ prepost + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noearlylate);
% base vs. prepost
...
% Full model
full = fitlme(nonan_Tbl,'mean ~ prepost + earlylate + (1|mouse) + (1|sess)'); 
disp(full)

% ~ PREPOST ~
prepost = fitlme(nonan_Tbl,'mean ~ prepost + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(prepost)
noprepost = fitlme(nonan_Tbl,'mean ~ earlylate + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noprepost);
% base vs. prepost
compare(base, prepost,'nsim',1000)
% full vs. noprepost 
...
disp(earlylate)
noearlylate = fitlme(nonan_Tbl,'mean ~ prepost + (1|mouse) + (1|sess)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noearlylate);
% base vs. prepost
compare(base, earlylate,'nsim',1000)
% full vs. noprepost 
