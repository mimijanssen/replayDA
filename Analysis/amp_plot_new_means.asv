...
r = 10000; % decimate by this amount. 


samples = minutes*60*refData.cfg.hdr{1,1}.SamplingFrequency/r;
samples_before = 4*60*refData.cfg.hdr{1,1}.SamplingFrequency/r;% 4 min before
samples_after = 11*60*refData.cfg.hdr{1,1}.SamplingFrequency/r; % ll min after
...
for i = 1:length(salineMice) % iterate through the mice
    ...
    for j = 1:numSessions % now open each of those sessions 
        ...
        deci_data = decimate(data.zdF_base, r); 
...
for iter = 1:length(fields_amp)
    ...
    figure(iter)
    plot(time_for_plotting,avg_amp)
end
...
%% statistics dF 
% change in dF : for each mouse, 1) find the max value in the pre injection period , 2)
% find the max value in the post injection period, 3) take the difference,
% 4) enter that into a t-test 

% 1) find the mean value in the post injection period : all samples after
% samples_before 
...
[h_c,p_c,stats_c] = chi2gof(dF_amp);
...
%% Edited Figure for lines between medians :
%
%
% 2) FIGURE TIME
figure(2);
...
count         = 0;
sal_medians   = zeros(1, length(fields_saline));
amp_medians   = zeros(1, length(fields_saline));
sal_positions = zeros(1, length(fields_saline));
amp_positions = zeros(1, length(fields_saline));

% Loop through each mouse
for iter = 1:length(fields_saline)

    % --- Saline ---
    count = count + 1;
    sal_positions(iter) = count;
    data_sal_sess = Saline_sess.(fields_saline{iter});
    sal_sess_dF   = mean(data_sal_sess(:, samples_before+1:end), 2) - data_sal_sess(:, samples_before);
    boxchart(count * ones(1, numel(sal_sess_dF)), reshape(sal_sess_dF, 1, []), ...
        'BoxFaceColor', med_c, 'MarkerStyle', 'none');
    sal_medians(iter) = median(sal_sess_dF);

    % --- Amphetamine ---
    count = count + 1;
    amp_positions(iter) = count;
    data_amp_sess = Amp_sess.(fields_saline{iter});
    amp_sess_dF   = mean(data_amp_sess(:, samples_before+1:end), 2) - data_amp_sess(:, samples_before);
    boxchart(count * ones(1, numel(amp_sess_dF)), reshape(amp_sess_dF, 1, []), ...
        'BoxFaceColor', high_c, 'MarkerStyle', 'none');
    amp_medians(iter) = median(amp_sess_dF);

end

% Connecting lines + per-mouse significance only
for iter = 1:length(fields_saline)

    % Reload data for stats
    data_sal_sess = Saline_sess.(fields_saline{iter});
    sal_dF        = mean(data_sal_sess(:, samples_before+1:end), 2) - data_sal_sess(:, samples_before);
    data_amp_sess = Amp_sess.(fields_saline{iter});
    amp_dF        = mean(data_amp_sess(:, samples_before+1:end), 2) - data_amp_sess(:, samples_before);

    % Connecting line between medians
    plot([sal_positions(iter), amp_positions(iter)], ...
         [sal_medians(iter),   amp_medians(iter)], ...
         'k-', 'LineWidth', 1.5);

    % Per-mouse significance
    [p_mouse, ~] = signrank(sal_dF, amp_dF,'tail','left');
    ...
    if ~isempty(sig_str)
        x_mid = mean([sal_positions(iter), amp_positions(iter)]);
        y_sig = max([sal_medians(iter), amp_medians(iter)]) + ...
                range([sal_medians, amp_medians]) * 0.08;
...
for iter = 1:length(fields_saline) % iterate through each mouse -- same for amphetamine...
    ...
    sal_sess_dF = mean(data_sal_sess(:,samples_before+1:end),2) - (data_sal_sess(:,samples_before)); % 2x1
...
for iter = 1:length(fields_saline) % iterate through each mouse -- same for amphetamine...
    ...
    % [dF_m_h, dF_m_p,~, dF_m_tstats] = ttest2(sal_dF_mice.(name), amp_dF_mice.(name)); % paired needs to be the same size 
     [dF_m_p, dF_m_h,dF_m_tstats] = signrank(sal_dF_mice.(name), amp_dF_mice.(name),'tail','left'); % paired needs to be the same size 
     disp(dF_m_h);
     disp(dF_m_p);
     disp(dF_m_tstats);
end

% %% ~~~~~~~~~~~~~  Permutation test per mouse ~~~~~~~~~~~~~~
% % this part of teh code 
% n_perms = 1000;
% rng(42);
% 
% mouse_names = fieldnames(Saline_sess);
% perm_results = struct();
% 
...
% 
%     real_sal = compute_mouse_stat(sal_data,samples_before);
%     real_amp = compute_mouse_stat(amp_data,samples_before);
% 
%     real_stat = mean([real_sal real_amp]);
% 
%     mouseFolder_sal = fullfile(salineFolder,mouseName);
%     mouseFolder_amp = fullfile(amphetamineFolder,mouseName);
% 
%     dataFiles_sal = dir(fullfile(mouseFolder_sal,'*processed*'));
%     dataFiles_amp = dir(fullfile(mouseFolder_amp,'*processed*'));
% 
%     rawSal = cell(length(dataFiles_sal),1);
%     rawAmp = cell(length(dataFiles_amp),1);
% 
%     fprintf('Loading recordings...\n')
% 
%     % ---------- Saline Data ----------
%     for j = 1:length(dataFiles_sal)
%         data = load(fullfile(mouseFolder_sal,dataFiles_sal(j).name));
%         % Decimate ONCE
%         rawSal{j} = decimate(data.zdF_base,r);
% 
%     end
% 
%     % ---------- Amphetamine Data ----------
%     for j = 1:length(dataFiles_amp)
%         data = load(fullfile(mouseFolder_amp,dataFiles_amp(j).name));
%         rawAmp{j} = decimate(data.zdF_base,r);
%     end
% 
%     % Permutation test
%     perm_stats = zeros(n_perms,1);
%     nSessions = length(rawSal)+length(rawAmp);
% 
%     for perm = 1:n_perms
%         sess_stats = zeros(nSessions,1);
%         k = 1;
%         % ================= Saline =================
%         for j = 1:length(rawSal)
%             x = rawSal{j};
%             n = length(x);
% 
...
%         sess_stats = sess_stats(1:k-1);
%         perm_stats(perm) = mean(sess_stats);
% 
%     end
% 
%     % Empirical p-value
%     p_two = (sum(abs(perm_stats)>=abs(real_stat))+1)/(n_perms+1); %  two-tailed test. 2 * P(TS  |ts| | H0 is true) = 2 * (1 - cdf(|ts|))
...
% 
%     perm_results.(mouseName).real_stat = real_stat;
%     perm_results.(mouseName).perm_stats = perm_stats;
%     perm_results.(mouseName).p_two = p_two;
%     perm_results.(mouseName).p_one = p_one;
% 
%     % Plot
%     figure;
% 
%     histogram(perm_stats,40,...
...
% 
%     xlabel('Mean post-injection DA - injection DA')
%     ylabel('Count')
%     title(sprintf('%s   p = %.4f',mouseName,p_two))
% 
%     set(gca,'FontSize',12)
%     box off
% 
...
%% NEW PERM TEST RESTRICT TO TIME INTEREST
%% ~~~~~~~~~~~~~  Permutation test per mouse SANDBOX ~~~~~~~~~~~~~~
n_perms = 1000;
rng(42);
...
% Amp_sess has each mouse in a structure with their decimated time periods 
% Saline_sess 
...
for i = 1:length(mouse_names)
    ...
    real_stat = real_amp - real_sal ; %mean([real_sal real_amp]);

    % Permutation test
    perm_stats = zeros(n_perms,1);
    nSessions_sal = size(sal_data,1)
    nSessions_amp = size(amp_data,1);

    for perm = 1:n_perms
        sess_stats_sal = zeros(nSessions_sal,1);
        k = 1;
        % ================= Saline =================
        for j = 1:size(sal_data,1)
            x = sal_data(j,:);
            n = length(x);
        ...
        sess_stats_amp = zeros(nSessions_amp,1);
        k = 1;
        % ================= Amphetamine =================
        for j = 1:size(amp_data,1)
