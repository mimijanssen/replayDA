...
%% SAVE SWR-DA Peak Size? 1 second....
% It's not really SWR-DA Strength. just peak size.  
SWR_DA_strength = {}; 
...
for s = 1:length(structure_names)
    ...
    SWR_DA_strength_all.(curr_structure_name) = mean([SWR_DA_strength.post, SWR_DA_strength.pre]); 
end
...
%% RPE STRENGTH over 4 seconds - instantaneous 
RPE_strength_dF = {}; 
...
for s = 1:length(structure_names)
    ...
    for i = 1:num_sessions
        ...
        % Pre-condition SWR-DA strength
        RPE_strength_dF_all(1,i) = mean(curr_structure.(session).dF_high) - mean(curr_structure.(session).dF_med  ); 
        % I actually want medium sized not omission. 
        % was previously taking the max...idk why but it doesn't change anything since there is one value. 
    end
    
    % Save the results in SWR_DA_strength_all under the current structure name
    RPE_strength_all.(curr_structure_name) = mean(RPE_strength_dF_all);
end

% RPE_strength_all has all of the RPE high - low. 

%% Find the percentage of RPE to FICTIVE 
mean(cell2mat(struct2cell(SWR_DA_strength_all)))*100/mean(cell2mat(struct2cell(RPE_strength_all)))
...
mean(SWR_DA_pre_all)
mean([SWR_DA_pre_all, SWR_DA_post_all])
% and std
