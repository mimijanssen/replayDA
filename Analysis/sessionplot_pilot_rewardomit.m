...
FP = load('M595_2025_10_05processed.mat'); % fiber data processed with my pipeline
load('M595_2025_10_05_recording14-manualIV.mat') % SWR intervals
load('M595_2025-10-05_track') % for pseudo_outcomes % CHANGE THIS 
...
% For left reward, right omit things are different... 
% evt3.t{1} left reg reward
% evt3.t{2} right nothing
...
left_pb = evt3.t{1} - csc_photo_fc.tvec(1); 
...
% For left omit right reward :)
right_c = [255, 165,0]./255;
left_c = [0,104,87]./255; 
...
reward = zeros(length(left_pb),window*2); %rows = trial, columns = signal 
omit = zeros(length(right_pb),window*2);
...
for iter = 1:1:2
    ...
    for ptime = 1:1:length(all_events{1,iter})
        ...
        end_trial = (indxpb + (window));
            if iter ==1 
                reward(ptime,:) = prepros_signal(init_trial:end_trial-1); 
                %h1 = plot((t(init_trial:end_trial))-t(init_trial), zdF(init_trial:end_trial), 'Color',[all_colors{1,iter}]);
            else iter == 2
                omit(ptime,:) = prepros_signal(init_trial:end_trial-1); 
...
% average by columns
if size(reward,1) > 1
    avg_high = mean(reward); 
    std_high = std(reward); 
elseif size(reward,1) == 1
    avg_high = reward;
    std_high = NaN; 
else
    avg_high = NaN;
...
if size(std_high,2) > 1
    shadedErrorBar(t_shared,avg_high,std_high,'lineprops',{'-','color',left_c,'MarkerFaceColor',left_c});
% if you ahve avghigh_ plot that too
hold on;
elseif size(avg_high,2) > 1
    plot(t_shared,avg_high,'LineWidth',2,'Color',left_c)
end
hold on;
if size(std_med,2) > 1
    shadedErrorBar(t_shared,avg_med,std_med,'lineprops',{'-','color',right_c,'MarkerFaceColor',right_c});
    hold on;
elseif size(avg_med,2) > 1
    plot(t_shared,avg_med,'LineWidth',2,'Color',right_c)
end
hold on;
...
saveas(fig,'M595_ontrack14_descriptive.png') % CHANGE THIS 
