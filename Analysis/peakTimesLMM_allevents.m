%% determining peak times and if there is an offset. 
%%
clc
clear
rng(10)
...
for i = 1:height(allTables2)
    ...
    allTables2.PostSWRPeak(i) = max(signal(4000:5000)- mean(signal(1:3000))); % NEED TO CHANGE THIS
    allTables2.PostSWRmean(i) = mean(signal(4000:5000)- mean(signal(1:3000)));
    [val, index] = max(signal(4000:5000)- mean(signal(1:3000)));
    allTables2.basepeaktime(i) = tvec_peak(index);
end

% Before SWR
allTables2.PreSWRPeak = NaN(height(allTables2),1);
allTables2.PreSWRmean = NaN(height(allTables2),1);
allTables2.Baseline = NaN(height(allTables2),1);
for i = 1:height(allTables2)
    if allTables2.PrePost(i) == 1
        signal = allTables2.TwosPreProc{i}.signal;
    elseif allTables2.PrePost(i) == 2
    ...
    allTables2.PreSWRPeak(i) = max(signal(3000:4000))- mean(signal(1:3000)); % 1:3000
    allTables2.PreSWRmean(i) = mean(signal(3000:4000))- mean(signal(1:3000)); %1:3000
    allTables2.Baseline(i) = mean(signal(1:3000)); %1:3000
...
allTables2.EarlyLate = reordercats(allTables2.EarlyLate, {'1','2'});

%
allTables2.PrePost = reordercats(allTables2.PrePost, ...
    ['1'; setdiff(categories(allTables2.PrePost), {'1'})]);

%% Mean and SEM of peak times for EarlyLate: 
...
for m = 1:length(mice)
    ...
    if any(early_idx)
        ...
        for s = 1:length(early_sessions)
            ...
            session_values(s) = ...
                mean(allTables2.basepeaktime(sess_idx),'omitnan');
        end
...
grand_early = median(mouse_early,'omitnan');
grand_late  = median(mouse_late,'omitnan');

fprintf('Early median = %.4f\n',grand_early);
fprintf('Late median  = %.4f\n',grand_late);
...
n_mice    = 8; % number of animals for SEM calculation
...
groups = {
    ...
    'Wake',           (allTables2.sleep == '1');
    'NREM',           (allTables2.sleep == '2');
...
%% ============================================================
% IMMOBILE SWR DETAILS
% Average sessions within mouse
% Then plot mouse averages
% =============================================================

mice = unique(track_immobile.mouseID);
n_mice = length(mice);

% Preallocate: rows = mice
swr_count_mouse = nan(n_mice,1);
swr_dur_mouse   = nan(n_mice,1);

sessions = unique(track_immobile.sess);
...
for m = 1:n_mice
    ...
    for s = 1:length(sessions)
        ...
        % Get all SWRs from this mouse/session
        idx = track_immobile.mouseID == mouse & ...
              track_immobile.sess == sess;

        %% SWR count
        count_this = sum(idx);

        %% SWR duration
        % Mean duration of SWRs within this session
        dur_values = track_immobile.SWRdur(idx);
        dur_this = mean(dur_values,'omitnan');

        % Store
        session_mouse(end+1,1) = double(mouse);
        session_id(end+1,1)    = sess;

        session_count(end+1,1) = count_this;
        session_dur(end+1,1)   = dur_this;

    end
end

% Average sessions within each mouse

for m = 1:n_mice
    mouse = mice(m);
    idx = session_mouse == double(mouse);
    swr_count_mouse(m) = mean(session_count(idx),'omitnan');
    swr_dur_mouse(m)   = mean(session_dur(idx),'omitnan');
end
...
scatter( ...
    repmat(x,1,n_mice) + randn(1,n_mice)*0.05, ...
    swr_count_mouse, ...
    60, ...
    'MarkerFaceColor','b', ...
    'MarkerEdgeColor','k', ...
...
h = boxchart(ones(size(swr_dur_mouse)), swr_dur_mouse, ...
    'BoxFaceColor', 'b');

hold on;

scatter( ...
    ones(size(swr_dur_mouse)) + randn(size(swr_dur_mouse))*0.05, ...
    swr_dur_mouse, ...
    60, ...
    'MarkerFaceColor', 'b', ...
    'MarkerEdgeColor', 'k', ...
...
%grand_mean_save = zeros(2,length(grand_mean));
...
for g = 1:size(group_defs2, 1)
    ...
    [grand_mean, grand_sd, n_events mean_mean] = compute_peth(allTables2, mask, n_samples);
    grand_sem = grand_sd / sqrt(n_mice);

    [val idx] =  max(grand_mean(4000:5000))
    tvec_peak(idx)
    %grand_mean_save(g,:) = grand_mean;
     % Shaded SEM
     % fill([tvec, fliplr(tvec)], ...
     %      [grand_mean + grand_sem, fliplr(grand_mean - grand_sem)], ...
     %       col, 'EdgeColor', 'none', 'FaceAlpha', 0.2);
...
ylabel('Base Mean DA (z-score)', 'FontSize', 13);
...
% Scatter colored by density
scatter(dur_s, peak_s, ...    % convert duration to ms
    8, density_sorted, ...
    'filled', 'MarkerFaceAlpha', 0.4, 'MarkerEdgeColor', 'none');
...
% Linear trend line
p        = polyfit(dur, peak, 1);
x_fit    = linspace(min(dur), max(dur), 200);
y_fit    = polyval(p, x_fit);
plot(x_fit, y_fit, 'w-',  'LineWidth', 3);   % white outline for contrast
plot(x_fit, y_fit, 'k--', 'LineWidth', 1.8);
...
ylabel('Baselined Mean DA (z-score)', 'FontSize', 13);
...
% Full Model 
full = fitlme(allTables2,'basepeaktime ~ PrePost  + EarlyLate+ sleep + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(full)
% BIC: 54479 
% swrID is sig, PrePost is sig but that's it. 

% Base Model 
base = fitlme(allTables2,'basepeaktime ~ 1 + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(base)

% ~ PREPOST ~
prepost = fitlme(allTables2,'basepeaktime ~ PrePost + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(prepost)
noprepost = fitlme(allTables2,'basepeaktime ~ EarlyLate + sleep + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noprepost);
% base vs. prepost
...
% ~ EarlyLate ~
earlylate = fitlme(allTables2,'basepeaktime ~ EarlyLate + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(earlylate)
noearlylate = fitlme(allTables2,'basepeaktime ~ PrePost + sleep + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noearlylate);
% base vs. prepost
...
% ~ WakeNrem ~
sleep = fitlme(allTables2,'basepeaktime ~ sleep + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(sleep)
nosleep = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nosleep);
% base vs. prepost
...
% ~ duration ~
dur = fitlme(allTables2,'basepeaktime ~ SWRdur + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(dur)
nodur = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + sleep + SWRpower + swrID + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nodur);
% base vs. prepost
...
% ~ swrpower ~
power = fitlme(allTables2,'basepeaktime ~ SWRpower + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(power)
nopower = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + sleep + SWRdur + swrID + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nopower);
% base vs. prepost
...
% ~ swrpowermax ~
powermax = fitlme(allTables2,'basepeaktime ~ SWRpmax + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(powermax)
full_max = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + sleep + SWRdur + SWRpmax + swrID +  (1|mouseID) ');%(ProcPeakTbl,'Peak ~
disp(full_max)
nopowermax = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + sleep + SWRdur + swrID + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nopowermax);
% base vs. prepost
compare(base, powermax,'nsim',1000)
% full vs. noprepost 
compare(nopowermax,full_max,'nsim',1000)

% ~ swrID ~
id = fitlme(allTables2,'basepeaktime ~ swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(id)
noid = fitlme(allTables2,'basepeaktime ~ PrePost + EarlyLate + sleep + SWRdur + SWRpower + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noid);
% base vs. prepost
...
% % BIC: 54479 
% % swrID is sig, PrePost is sig but that's it. 
% disp(crosstab(allTables2.EarlyLate, allTables2.sleep))
% 
% % Base Model 
...
% full_history = fitlme(allTables2,'Base_peak ~ PrePost + history  + EarlyLate+ sleep + SWRdur + SWRpower + swrID + (1|mouseID) ');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
% disp(full_history)

% history = fitlme(allTables2,'Base_peak ~ history + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
% disp(history)
% nohistory = fitlme(allTables2,'Base_peak ~ PrePost + EarlyLate + sleep + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
% disp(noprepost);
% % base vs. prepost
...
% Base Model 
base = fitlme(allTables2,'Base_mean ~ 1 + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(base)
% BIC 54442
...
% ~ WakeNrem ~
sleep = fitlme(allTables2,'Base_mean ~ sleep + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(sleep)
nosleep = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + SWRdur + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nosleep);
% base vs. prepost
compare(base, sleep,'nsim',1000)
% full vs. noprepost 
...
% ~ duration ~
dur = fitlme(allTables2,'Base_mean ~ SWRdur + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(dur)
nodur = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + sleep + SWRpower + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nodur);
% base vs. prepost
compare(base, dur,'nsim',1000)
% full vs. noprepost 
...
% ~ swrpower ~
power = fitlme(allTables2,'Base_mean ~ SWRpower + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(power)
nopower = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + sleep + SWRdur + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nopower);
% base vs. prepost
compare(base, power,'nsim',1000)
% full vs. noprepost 
...
% ~ swrpowermax ~
powermax = fitlme(allTables2,'Base_mean ~ SWRpmax + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(powermax)
fullmax = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + sleep + SWRdur + SWRpmax + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(fullmax);
nopowermax = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + sleep + SWRdur + swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(nopowermax);
% base vs. prepost
compare(base, powermax,'nsim',1000)
% full vs. noprepost 
compare(nopowermax,fullmax,'nsim',1000)

% ~ swrID ~
id = fitlme(allTables2,'Base_mean ~ swrID + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(id)
noid = fitlme(allTables2,'Base_mean ~ PrePost + EarlyLate + sleep + SWRdur + SWRpower + (1|mouseID)');%(ProcPeakTbl,'Peak ~ BeforeAfter + swrID + PrePost + (1|mouseID) + (1|sess)');
disp(noid);
% base vs. prepost
compare(base, id,'nsim',1000)
% full vs. noprepost 
...
% Using ksdensity-based violins since you may not have the violin toolbox
%%
function plot_violins(allTables2, outcome_var, title_str)
    % Define grouping variables and their labels
    groupings = {
        'PrePost',   {'Pre','Post'},   [1 2];
        'sleep',     {'Wake','NREM'},  {'1','2'};
        'EarlyLate', {'Early','Late'}, [1 2];
    };

    n_groups = size(groupings, 1);
    colors = {[0.4 0.6 0.8], [0.9 0.4 0.3], [0.4 0.8 0.5], [0.8 0.6 0.2]};
    ...
    for g = 1:n_groups
        ...
        var_name   = groupings{g, 1};
        grp_labels = groupings{g, 2};
        grp_vals   = groupings{g, 3};

        y     = allTables2.(outcome_var);
        valid = ~isnan(y);

        x_positions = 1:length(grp_labels);

        % Collect group data for stats
        grp_data_all = cell(length(grp_labels), 1);

        for k = 1:length(grp_vals)
            % Get mask for this group
            if iscategorical(allTables2.(var_name))
                if iscell(grp_vals)
                    mask = allTables2.(var_name) == grp_vals{k} & valid;
                else
                    mask = allTables2.(var_name) == string(grp_vals(k)) & valid;
                end
            else
                mask = allTables2.(var_name) == grp_vals(k) & valid;
            end
            ...
            grp_data = y(mask);
            grp_data = grp_data(~isnan(grp_data));
            grp_data_all{k} = grp_data;

            if length(grp_data) < 10
                continue
            end
            ...
            % KDE violin
            [f, xi] = ksdensity(grp_data, 'NumPoints', 100);

            % Clip to 1st-99th percentile
            in_range = xi >= prctile(grp_data, 1) & xi <= prctile(grp_data, 99);
            f  = f(in_range);
            xi = xi(in_range);
            ...
            % Draw violin — no box
            fill([xpos + f, xpos - fliplr(f)], ...
                 [xi,        fliplr(xi)], ...
                 col, 'FaceAlpha', 0.5, 'EdgeColor', 'none');

            % Median line only
            med = median(grp_data);
            plot([xpos - 0.15, xpos + 0.15], [med med], ...
                 'k-', 'LineWidth', 2.5);
           

            % Add median value as text
            text(xpos + 0.2, med, sprintf('%.4f', med), ...
            'VerticalAlignment', 'middle', 'FontSize', 9, 'Color', 'k'); 
            
            % Jittered points
            n_pts = min(length(grp_data), 300);
            idx_s = randperm(length(grp_data), n_pts);
            jitter = (rand(n_pts, 1) - 0.5) * 0.1;
            scatter(xpos + jitter, grp_data(idx_s), ...
                8, 'k', 'filled', 'MarkerFaceAlpha', 0.15);

            % n label
            text(xpos, min(xi) - range(xi)*0.05, ...
                 sprintf('n=%d', length(grp_data)), ...
                 'HorizontalAlignment', 'center', 'FontSize', 9);
        end

        %% Significance bars between all pairs
        % collect y max for bracket placement
        y_vals = y(valid);
        y_top  = prctile(y_vals, 99);
        y_range = range(y_vals(y_vals >= prctile(y_vals,1) & y_vals <= prctile(y_vals,99)));
        bracket_step = y_range * 0.08;
        bracket_y    = y_top + bracket_step;

        pairs = nchoosek(1:length(grp_labels), 2); % all pairwise combos
       ...
       for p = 1:size(pairs, 1)
            ...
            n1 = length(d1);
            n2 = length(d2);
            r_effect = 1 - (2 * stats_rs.ranksum) / (n1 * (n1 + n2 + 1));
            obs_diff = median(d1) - median(d2);
            ...
            ci_lo = prctile(boot_diff, 2.5);
            ci_hi = prctile(boot_diff, 97.5);
            ...
            plot([x1, x1, x2, x2], ...
                 ...
                 'k-', 'LineWidth', 1);
            text((x1 + x2)/2, bracket_y + tick_h * 0.5, sig_str, ...
                 'HorizontalAlignment', 'center', 'FontSize', 11);

            % Add effect size below sig star on plot
            text((x1 + x2)/2, bracket_y + tick_h * 2, ...
                 sprintf('r=%.2f', r_effect), ...
                 'HorizontalAlignment', 'center', 'FontSize', 8, ...
                 'Color', [0.4 0.4 0.4]);

            bracket_y = bracket_y + bracket_step * 1.8;
       end
        ...
        % Axes
        xticks(x_positions);
        xticklabels(grp_labels);
        xlim([0.5, length(grp_labels) + 0.5]);
        ylabel(strrep(outcome_var, '_', ' '));
    ...
    fprintf('%-12s %10s %10s %10s %10s\n', 'Group', 'Median', 'Mean', 'IQR_lo', 'IQR_hi');
    for k = 1:length(grp_vals)
        d = grp_data_all{k};
        if isempty(d)
            continue; 
        end
        fprintf('%-12s %10.5f %10.5f %10.5f %10.5f\n', ...
        grp_labels{k}, ...
        median(d), ...
        mean(d), ...
        prctile(d, 25), ...
...
function [grand_mean, grand_sd, n_events mean_mean] = compute_peth(allTables2, mask, n_samples)
    ...
    for i = 1:length(idx)
        ...
        if allTables2.PrePost(row) == '1'
            sig = single(allTables2.TwosPreProc{row}.signal(:)')-allTables2.Baseline(row);   % single precision
        ...
        elseif allTables2.PrePost(row) == '2'
            sig = single(allTables2.TwosPostProc{row}.signal(:)')-allTables2.Baseline(row);  % single precision
