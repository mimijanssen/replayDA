...
% input: preprocessed fiber data, detected SWRs, good LFP csc, and DLC points 
...
clear; clc;
rng(pi)
cd 'C:\Data\M433\M433_2023_09_19_recording1'; 
FP = load('M433_2023_09_19processed.mat'); % fiber data processed with my pipeline
FP_Tlab = load('M433_2023_09_19Tlabprocessed.mat'); % fiber data processed with Tritsch lab  
load('2023-09-19_M433_recording1detectedSWRs.mat') % SWR intervals

%% load detected SWRs 
% To return: pre_count and post_count 
% --------------------------------------------------------------------------
...
lfp = csc.data;
time = FP_Tlab.tvec - FP_Tlab.tvec(1); %FP.tvec;%t; 

FS = 100; % csc.cfg.hdr{1}.SamplingFrequency; % set FP_data.acq.Fs to sampling frequency rate (5000 points per second) 
% sampling rate for trisch is 100... need to save those parameters. (also
% it cuts the first 15 seconds 

%%%%%%%% FIGURE OUR WHY FS IS NOT SAVED ALONG WITH PARAMS
...
for i = 1:1:size(SWR_ind_mid)
    ...
    SWR_fiber_ind(i) = nearest_idx3(SWR_time_mid(i),time); % find the corresponidng time in fiber and saves the index.
end
...
prepros_signal = FP_Tlab.data;%FP.zF_win_60s_no; 

seconds = 8; % edit this
samples = (seconds*FS)/2;  % divide by two because you want 4s + and - directions 

zdF_win1_extract = zeros(length(SWR_ind_mid), seconds*FS+1); 
time_win1_extract =  zeros(length(SWR_ind_mid), seconds*FS+1);
...
zdF_extract_post = zeros(length(post_SWR_ind), seconds*FS+1); 
time_extract_post =  zeros(length(post_SWR_ind), seconds*FS+1);
...
% initialize 
circ_zdF_extract = zeros(events_num, seconds*FS+1); 
avg_circ_zdF_extract_post = zeros(N,seconds*FS+1);
...
% PETH ----------------------------------------------------------------_
zdF_extract_pre = zeros(length(pre_SWR_ind), seconds*FS+1); 
time_extract_pre =  zeros(length(pre_SWR_ind), seconds*FS+1);
...
% initialize 
circ_zdF_extract = zeros(events_num, seconds*FS+1); 
avg_circ_zdF_extract_pre = zeros(N,seconds*FS+1);
...
zlfp = zscore_tsd(csc);
% the tvec here is not initialized so use exp keys here 
...
[S_pre,F_pre,T_pre,P_pre] = spectrogram(zlfpr_pre.data,hanning(7500),3750,1:20,csc.cfg.hdr{1,1}.SamplingFrequency); % FS
...
% 
% posx_pre_start = nearest_idx3(prerecord_init,fpos.tvec);
% posx_pre_end = nearest_idx3(prerecord_end,fpos.tvec);
% posx_post_start = nearest_idx3(postrecord_init,fpos.tvec);
% posx_post_end = nearest_idx3(postrecord_end,fpos.tvec); % fpos is initialized
...
load('M433_2023-09-19_track.mat') % for pseudo_outcomes % CHANGE THIS 
LoadExpKeys();
cfg_evt = [];
...
window = 4*FS; % automate based on sampling frequency 8seconds/ 
t = [];
t = FP_Tlab.tvec- FP_Tlab.tvec(1);
...
for iter = 1:1:3
    ...
    for ptime = 1:1:length(all_events{1,iter})
        indxpb = find(abs(t-all_events{1,iter}(ptime)) < 0.01); % find the time of the event. 0.0005
        init_trial = (indxpb - (window)); % 8 seconds; remember 0.0002 is the time in seconds for each voltage point
        end_trial = (indxpb + (window));
        %if abs(max(prepros_signal(init_trial:end_trial))) + abs(min(prepros_signal(init_trial:end_trial))) < 100 % changed from 30 to 100
       ...
       % else
         ...
         % decided to keep artifacts... is this the only one? 
       % end
    end
end
...
%% Pwelch 
wsize = FS * 4; % sampling frequency * 4s is the amount of samples (the window size needed)
FP_FS = csc_photo_fc.cfg.hdr{1,1}.SamplingFrequency; % I DID NOT UPDATE THIS SAMPLING FREQUENCY CORRECTLY...
wsize2 = FP_FS*4;
[Pxx,F] = pwelch(csc.data,hanning(wsize),wsize/2,[],csc.cfg.hdr{1,1}.SamplingFrequency);
[Pxx_f,F_f] = pwelch(csc_photo_fc.data,hanning(wsize2),wsize2/2,[],FP_FS);


%% restrict signal for pwelch
% needed times

% does not need prerecord_init, it needs ExpKeys times ...
...
% use this for recording 2 

% use this for others
[Pxx_fpre,F_fpre] = pwelch(fiber_pre.data,hanning(wsize2),wsize2/2,[],FP_FS);
[Pxx_ftrack,F_ftrack] = pwelch(fiber_track.data,hanning(wsize2),wsize2/2,[],FP_FS);
[Pxx_fpost,F_fpost] = pwelch(fiber_post.data,hanning(wsize2),wsize2/2,[],FP_FS);
...
% swr examples
% 5 - 25
% cfg3 = []; cfg3.fc = ExpKeys.goodSWR(1);
% cfg3.display = 'iv';
% cfg3.mode = 'center';
% cfg3.fgcol = 'k';
% lfp = LoadCSC(cfg3);
% subset_SWR = evt;
% subset_SWR.evt.tstart = evt.tstart(1:20);
% subset_SWR.evt.tend = evt.tend(1:20);
% subplot(4,8,5:25)
% PlotTSDfromIV_subplot(cfg3,subset_SWR.evt,lfp)

% swr count 
% 26
% subplot(3,3,4)
% bar(swr_label,swr_count)
% title('SWR Count')
...
% subplot(3,4,6)
% plot(F_f,10*log10(Pxx_f),'k'); xlabel('Frequency (Hz)'); ylabel('Power (dB)');
% xlim([0 100]);
% title('PSD for Fiber')
...
% plot(F_f,10*log10(Pxx_f),'k'); xlabel('Frequency (Hz)'); ylabel('Power (dB)');
% xlim([0 10]);
% title('PSD for Fiber (lower f)')

subplot(3,4,2)
plot(F_fpre,10*log10(Pxx_fpre),'k'); xlabel('Frequency (Hz)'); ylabel('Power (dB)');
xlim([0 10]);
title('PSD for Fiber (pre-track)')
...
subplot(3,4,10)
plot(F_fpost,10*log10(Pxx_fpost),'k'); xlabel('Frequency (Hz)'); ylabel('Power (dB)');
xlim([0 10]);
title('PSD for Fiber (post-track)')
...
% fiber after SWR subplot
% figure(4)
% shadedErrorBar(time_win1_extract(1,:),avg_fiber_win1,std_fiber_win1,'lineProps','-k','transparent',1)
% hold on
% plot(time_win1_extract(1,:),avg_fiber_win1)
% xl = xline(4,'-',{'SWR'});
...
% xticks([0 4 8])
% xticklabels({'-4','0','4'})
% title('detrended and z-scored signal PETH Centered on Detected SWRs (10 s)')
% ylabel('Averaged Signal (detrended & z-scored)')
% xlabel('Time (s)')
...
%31
% subplot(3,4,8)
% bar({'pre','post'},[speed_pre,speed_post])
% title('Median Speed')
% ylabel('Median Speed (pixels/s)')
