function [dF,baseline] = detrendFP(FP,interpType,fitType,basePrc,winSize,winOv,Fs)
...
nPts = floor(Ls/(winSize-winOv)); %Determine number of baseline points to be found (based on window size)

%X is a vector of positional points in the data that we will be gathering baseline points
%length of the FP (samples if you didn't subsamples to seconds 
...
for n = 0:nPts-1
    ...
    Y(n+1) = prctile(FP(I1:I2),basePrc); %finds the fifth percentilel in the time window specifed 
end

interpFit = interp1(X,Y,L,interpType,'extrap'); %Create an interpolated line using the previously calculated baseline points
%interpolate a line between each data point and extrapolate if there are no
...
dF = (FP-baseline)./baseline; % substract the baseline line to detrend and divide by baseline to normalize the data
dF = dF*100; % multiply by 100
