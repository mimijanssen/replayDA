...
pre_count = [RPE.RPE1.swr_count(1); RPE.RPE2.swr_count(1); RPE.RPE3.swr_count(1); RPE.RPE4.swr_count(1);RPE.RPE5.swr_count(1); RPE.RPE6.swr_count(1); RPE.RPE7.swr_count(1)];  
post_count = [RPE.RPE1.swr_count(3); RPE.RPE2.swr_count(3); RPE.RPE3.swr_count(3); RPE.RPE4.swr_count(3);RPE.RPE5.swr_count(2); RPE.RPE6.swr_count(2);RPE.RPE7.swr_count(2)];  
...
% high 
sess_high = [RPE.RPE1.avg_high; RPE.RPE2.avg_high; RPE.RPE3.avg_high; RPE.RPE4.avg_high;RPE.RPE5.avg_high; RPE.RPE6.avg_high; RPE.RPE6.avg_high; RPE.RPE8.avg_high; ];  %RPE.RPE8.avg_high
mean_high = mean(sess_high);
std_high = std(sess_high);
% medium
sess_med = [RPE.RPE1.avg_med; RPE.RPE2.avg_med; RPE.RPE3.avg_med; RPE.RPE4.avg_med;RPE.RPE5.avg_med; RPE.RPE6.avg_med; RPE.RPE7.avg_med; RPE.RPE8.avg_med;];  % RPE.RPE8.avg_med
...
figure(1)
shadedErrorBar(RPE.RPE1.t_shared,mean_high,std_high,'lineprops',{'-','color',high_c,'MarkerFaceColor',high_c});
hold on
plot(RPE.RPE1.t_shared,mean_high,'LineWidth',3,'Color',high_c)
hold on
shadedErrorBar(RPE.RPE1.t_shared,mean_med,std_med,'lineprops',{'-','color',med_c,'MarkerFaceColor',med_c});
hold on
plot(RPE.RPE1.t_shared,mean_med,'LineWidth',3,'Color',med_c)
shadedErrorBar(RPE.RPE1.t_shared,mean_low,std_low,'lineprops',{'-','color',low_c,'MarkerFaceColor',low_c});
plot(RPE.RPE1.t_shared,mean_low,'LineWidth',3,'Color',low_c)
...
sess_fiber_pre = [sess.sess1.avg_fiber_pre;sess.sess2.avg_fiber_pre;sess.sess3.avg_fiber_pre; sess.sess4.avg_fiber_pre;sess.sess5.avg_fiber_pre;sess.sess6.avg_fiber_pre;sess.sess7.avg_fiber_pre;]; %sess.sess8.avg_fiber_pre
...
sess_fiber_post = [sess.sess1.avg_fiber_post;sess.sess2.avg_fiber_post;sess.sess3.avg_fiber_post;sess.sess4.avg_fiber_post;sess.sess5.avg_fiber_post;sess.sess6.avg_fiber_post;sess.sess7.avg_fiber_post;]; %sess.sess8.avg_fiber_post
sess_circ_track = [sess.sess1.circ_avg_track;sess.sess2.circ_avg_track;sess.sess3.circ_avg_track;sess.sess4.circ_avg_track;];%sess.sess5.circ_avg_track;sess.sess6.circ_avg_track;sess.sess7.circ_avg_track;]; %sess.sess8.circ_avg_post
sess_fiber_track = [sess.sess1.avg_fiber_track;sess.sess2.avg_fiber_track;sess.sess3.avg_fiber_track;sess.sess4.avg_fiber_track;]; %sess.sess8.avg_fiber_post
...
xticks([0 4 8])
xticklabels({'-4','0','4'})
title('M548: Pre Track Rest PETH')
...
%hold on
shadedErrorBar(sess.sess1.time(1,:),avg_fiber_post,std_fiber_post,'lineProps','-g','transparent',1)
hold on
...
xticks([0 4 8])
xticklabels({'-4','0','4'})
title('M548: Post Track Rest PETH')
ylabel('Averaged Signal (zdF)')
xlabel('Time from SWR (s)')
legend('','signal','','signal','Location','northwest')
legend boxoff 
...
%sess_high = [RPE.RPE1.avg_high; RPE.RPE2.avg_high; RPE.RPE3.avg_high; ]; %RPE.RPE4.avg_high; RPE.RPE5.avg_high; RPE.RPE6.avg_high; RPE.RPE7.avg_high; ];  %RPE.RPE8.avg_high
%mean_high = mean(sess_high);
%std_high = std(sess_high);

%sess_med = [RPE.RPE1.avg_med; RPE.RPE2.avg_med; RPE.RPE3.avg_med; ]; %RPE.RPE4.avg_med; RPE.RPE5.avg_med; RPE.RPE6.avg_med; RPE.RPE7.avg_med;];  % RPE.RPE8.avg_med
%mean_med = mean(sess_med);
%std_med = std(sess_med);

%sess_low = [RPE.RPE1.avg_low; RPE.RPE2.avg_low; RPE.RPE3.avg_low; ]; %RPE.RPE4.avg_low; RPE.RPE5.avg_low; RPE.RPE6.avg_low; RPE.RPE7.avg_low; ];  %RPE.RPE8.avg_low
%mean_low = mean(sess_low);
%std_low = std(sess_low);
%RPE.RPE1.t_shared
...
save(filename, 'pre_count', 'post_count', 'mean_pre', 'mean_post', 'std_pre', 'std_post','avg_fiber_post','avg_fiber_pre','std_fiber_post','std_fiber_pre','sess');